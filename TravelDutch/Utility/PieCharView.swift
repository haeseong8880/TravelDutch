...
//
// 출처 블로그 : https://sihyungyou.github.io/iOS-PieChart/
import UIKit
...
class PieChartView: UIView {
    ...
    func animateChart() {
        ...
        if slices != nil && slices!.count > 0 {
            ...
            addSlice(firstSlice)
            if(slices![sliceIndex].percent >= 0.05){
                addLabel(firstSlice)
                addtext(firstSlice)
            }
        }
...
extension PieChartView {
    private func addtext(_ slice: Slice) {
        let center = self.center
        // 현재 퍼센트와 그릴만큼의 퍼센트의 중간 위치를 찾고 거기에 레이블을 추가한다
        let labelCenter = getLabelCenter(currentPercent, currentPercent + slice.percent)
        ...
        label.text = slice.text
        
        label.translatesAutoresizingMaskIntoConstraints = false
        NSLayoutConstraint.activate([label.centerXAnchor.constraint(equalTo: self.centerXAnchor, constant: labelCenter.x - center.x),
                                     label.centerYAnchor.constraint(equalTo: self.centerYAnchor, constant: labelCenter.y - center.y - 20)])
